...
这是 `宇宙模型 v0.2` 的独立版本目录，初始代码基于 `宇宙模型 v0.1` 拷贝生成，后续可单独演化。

该版本与工作区共用：

- `.env`：模型与服务环境变量
- `.local_rag`：本地 RAG 存储
- `/Users/Min369/Documents/同步空间/Manju/AIProjects/UnivModel/RAG/` 下的参考资料
- `/Users/Min369/Documents/同步空间/Manju/AIProjects/UnivModel/行星运动模型/` 下的星历与模型文件

默认端口与 `v0.1` 错开，便于两个版本并行运行：
...
- 版本首页：`http://127.0.0.1:5174/?home=1`
- 智能体工作台：`http://127.0.0.1:5174/`
...
- OCR 校正测试台：`http://127.0.0.1:5174/?ocr=1`
...
```bash
APP_PORT=8801 FRONTEND_PORT=5201 ./start_all.sh
```
...
```bash
curl -s "http://127.0.0.1:8788/api/library/config"
```

列出本地 Markdown 分类：

```bash
curl -s "http://127.0.0.1:8788/api/library/categories"
```

列出某个分类下的 Markdown 文档：

```bash
curl -s "http://127.0.0.1:8788/api/library/documents?category=切比雪夫多项式插值md&limit=20"
```

先预览某个分类将摄入哪些文档，不真正写入 RAG：

```bash
curl -s "http://127.0.0.1:8788/api/library/ingest" \
...
```bash
curl -s "http://127.0.0.1:8788/api/ocr/correct" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "http://127.0.0.1:8788/api/ocr/convert-and-correct" \
  -H "Content-Type: application/json" \
  -d '{
...
- `http://127.0.0.1:8788/model-tester.html`
