...
def _pdf_pages(content: bytes, page_number: int | None, max_pages: int, zoom: float) -> tuple[list[dict[str, Any]], int]:
...
class OcrPreviewService:
    def preview_pages(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        max_pages = max(1, min(int(payload.get("maxPages") or 20), 50))
        ...
        try:
            if mime_type == "application/pdf" or name.lower().endswith(".pdf"):
                pages, total_pages = _pdf_pages(content, page_number=page_number, max_pages=max_pages, zoom=zoom)
