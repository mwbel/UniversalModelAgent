...
function V02HomePage() {
  const workspaceHref = buildWorkspaceUrl()
  const galleryHref = buildGalleryPageUrl()
  const ocrHref = buildOcrWorkbenchUrl()

  return (
    <main className="v02-home">
      <section className="v02-hero">
        <div>
          <p className="eyebrow">Universe Model v0.2</p>
          <h1>宇宙模型 v0.2</h1>
          <p>
            面向本地优先知识检索、天文学问答、A2UI 可视化和 OCR 校正流程的新版本起点。
          </p>
        </div>
        <a className="v02-primary-link" href={workspaceHref}>
          进入工作台
        </a>
      </section>

      <section className="v02-launch-grid">
        <a className="v02-launch-card" href={workspaceHref}>
          <span>Chat</span>
          <strong>智能体问答工作台</strong>
          <p>默认优先检索本地项目资料，回答中可自动挂载 A2UI 可视化。</p>
        </a>
        <a className="v02-launch-card" href={galleryHref}>
          <span>Gallery</span>
          <strong>天文学展示厅</strong>
          <p>集中查看 3D 天球、DE440 计算器、食相演示和交互组件。</p>
        </a>
        <a className="v02-launch-card" href={ocrHref}>
          <span>OCR</span>
          <strong>OCR 校正测试台</strong>
          <p>测试 MinerU Markdown 高风险块检测、截图辅助校正与结果合并。</p>
        </a>
      </section>
    </main>
  )
