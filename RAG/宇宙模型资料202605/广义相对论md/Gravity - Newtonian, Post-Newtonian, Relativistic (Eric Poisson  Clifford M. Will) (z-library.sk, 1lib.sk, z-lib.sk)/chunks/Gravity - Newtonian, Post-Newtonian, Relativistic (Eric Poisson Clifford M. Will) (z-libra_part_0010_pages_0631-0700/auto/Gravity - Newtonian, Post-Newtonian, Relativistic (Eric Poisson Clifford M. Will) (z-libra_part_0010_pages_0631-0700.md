![](images/8bc8d31b769f3e0764d64e6b4349d53f041441d8f885c653f3d535311b7d5b54.jpg)  
...
The vectors $\boldsymbol { e } ^ { \prime } { } _ { 1 }$ and $\boldsymbol { e ^ { \prime } } _ { 2 }$ are transverse to the direction of propagation $N = - e _ { 3 } ^ { \prime }$ , but they are not equal to the transverse vectors $e _ { X }$ and $e _ { Y }$ . Indeed, these vectors will in general be
...
The quadrupole-formula controversy described in $\mathrm { B o x } \ 1 1 . 2$ is related in much more detail in Daniel Kennefick’s wonderful book Traveling at the Speed of Thought, published in 2007; the phrase is due to Eddington (1922), who was referring specifically to the unphysical gauge modes contained in the theory. The Einstein–Rosen paper eventually appeared in 1937 in the Journal of the Franklin Institute, a publication that specializes in engineering and applied mathematics; the identity of the mysterious Physical Review referee was finally revealed by Kennefick in his 2005 Physics Today article. Another reference cited in $\mathrm { B o x } 1 1 . 2$ is Ehlers et al. (1976), and a useful summary of the controversy is presented in Walker and Will (1980).
...
$$
\begin{array} { c } { { \ell _ { \alpha } = - c \partial _ { \alpha } ( t - R / c ) , \qquad n _ { \alpha } = - \frac 1 2 c \partial _ { \alpha } ( t + R / c ) , } } \\ { { \ell _ { \alpha } \ell ^ { \alpha } = n _ { \alpha } n ^ { \alpha } = m _ { \alpha } m ^ { \alpha } = \bar { m } _ { \alpha } \bar { m } ^ { \alpha } = 0 , } } \\ { { \ell _ { \alpha } n ^ { \alpha } = - 1 , \qquad m _ { \alpha } \bar { m } ^ { \alpha } = 1 , } } \\ { { \eta ^ { \alpha \beta } = - 2 \ell ^ { ( \alpha } n ^ { \beta ) } + 2 m ^ { ( \alpha } \bar { m } ^ { \beta ) } . } } \end{array}
$$
...
where $H ^ { \alpha \mu \beta \nu } : = \mathfrak { g } ^ { \alpha \beta } \mathfrak { g } ^ { \mu \nu } - \mathfrak { g } ^ { \alpha \mu } \mathfrak { g } ^ { \beta \nu }$ . We wish to formulate these equations in a gauge that is not harmonic. Instead we choose to impose the gauge conditions
...
(b) Letting $h _ { + } = A _ { + }$ cos ωt and $h _ { \times } = A _ { \times } \sin \omega t$ , calculate $\scriptstyle { \frac { 1 } { 2 } } m \langle ( d \delta \xi / d t ) ^ { 2 } \rangle$ , the timeaveraged kinetic energy of the particle, as a function of $A _ { + }$ , $A _ { \times } , \omega$ , and direction.
...
$$
\begin{array} { l } { { v _ { \infty } = \sqrt { \displaystyle { \frac { G m } { p } } } e \biggl [ 1 - { \frac { 1 } { 2 } } e ^ { - 2 } + O ( e ^ { - 4 } ) \biggr ] , } } \\ { { r = \displaystyle { \frac { b } { \cos { \phi } } } \biggl [ 1 - \frac { 1 } { e \cos { \phi } } + O ( e ^ { - 2 } ) \biggr ] , } } \\ { { v = v _ { \infty } \bigl [ - e ^ { - 1 } \sin { \phi } , 1 + e ^ { - 1 } \cos { \phi } , 0 \bigr ] + O ( e ^ { - 2 } ) . } } \end{array}
$$
...
(c) Using the quadrupole formula, and taking the waves to be propagating in the direction of the vector $N = [ \sin \vartheta \cos \varphi$ , sin $\vartheta$ sin ϕ, cos ϑ], show that the gravitational-wave polarizations are given by
...
11.8 Show that the angular pattern functions for an interferometer whose arms make an angle $\chi$ with each other are the same as in Eqs. (11.320), but multiplied by sin χ . Hint: Orient the arms in the 1-2 plane so that each one makes an angle $\textstyle { \frac { \pi } { 4 } } - { \frac { 1 } { 2 } } \chi$ with respect to the $e _ { 1 }$ and $\scriptstyle { e _ { 2 } }$ axes.
...
The total dipole moment can also be expressed as $\begin{array} { r } { I _ { e } = \sum _ { A } \int _ { A } \rho _ { e } ( \pmb { r } _ { A } + \pmb { \bar { x } } ) d ^ { 3 } \pmb { \bar { x } } } \end{array}$ , with $\bar { x }$ denoting the position of a fluid element relative to the center-of-mass of each body. Integration of $\rho _ { e } \bar { \boldsymbol { x } }$ produces zero when the charge distribution is reflection-symmetric about the center-of-mass, and under these circumstances we obtain the simple expression

$$
I _ { e } = \sum _ { A } q _ { A } { { r } _ { A } }
$$
...
$$
m _ { A } a _ { A } ^ { j } = - \int _ { A } \rho _ { e } \partial _ { j } U _ { e } d ^ { 3 } x ^ { \prime } + O ( c ^ { - 2 } ) ,
$$
...
$$
\partial _ { j } U _ { e , - \it A } ( t , { \bf r } _ { A } ) = - \sum _ { B \ne A } \kappa q _ { B } \frac { n _ { A B } ^ { j } } { r _ { A B } ^ { 2 } } ,
$$
...
Because the radiated power $\mathcal { P }$ scales as $c ^ { - 3 }$ to leading order in a post-Coulombian expansion, our improved treatment of the equations of motion must include terms of order $c ^ { - 3 }$ in order to account for radiation-reaction effects. A systematic post-Coulombian expansion of the equations of motion would therefore commence with Eq. (12.18) at leading order, then incorporate 1pc terms at order $c ^ { - 2 }$ , 1.5pc terms at order $c ^ { - 3 }$ , 2pc terms at order $c ^ { - 4 }$ , and so on. Here we shall jump directly from 0pc order to $1 . 5 \mathrm { { p c } }$ order, and bypass the additional conservative terms that appear at 1pc order; we shall not pursue the post-Coulombian expansion beyond this point.
...
$$
\begin{array} { l } { \displaystyle \Phi _ { \mathrm { r r } } = \frac { \kappa } { 6 c ^ { 3 } } \Big ( 2 \boldsymbol { x } \cdot \dddot { \boldsymbol { I } _ { e } } - \dddot { \boldsymbol { I } } _ { e } ^ { k k } \Big ) + O ( c ^ { - 5 } ) , } \\ { \displaystyle } \\ { \displaystyle \boldsymbol { A } _ { \mathrm { r r } } = - \frac { \kappa } { c ^ { 3 } } \ddot { \boldsymbol { I } } _ { e } + O ( c ^ { - 5 } ) , } \end{array}
$$
...
$$
\frac { d E } { d t } = \frac { d } { d t } \bigg ( \frac { 2 \kappa } { 3 c ^ { 3 } } \ddot { \pmb { I } } _ { e } \cdot \pmb { \dot { I } } _ { e } \bigg ) - \frac { 2 \kappa } { 3 c ^ { 3 } } \big | \ddot { \pmb { I } } _ { e } \big | ^ { 2 }
$$
...
<table><tr><td>Box 12.1</td><td>Redefining the energy</td></tr><tr><td></td><td>The reader might feel that it is inappropriate to redefine the energy E for the sole purpose of obtaining a desired statement of energy balance. We wish to reassure the reader: there was nothing sacred about the initial definition of the system&#x27;s energy,and aredefinition can be entirely appropriate.It is helpful to recall thatthe energy is notsome rigid quantity defined a priori,but thatits definitionis provided bythedynamics ofthe mechanicalsystem.Fora conservative system,it is whateverfunctionofthe positions and velocities that happens to be conserved. Because the zero point of energy is notfixed (at least at the non-quantum level), the definition is not even unique.</td></tr></table>
...
that preserves the harmonic gauge conditions $\partial _ { \beta } h ^ { \alpha \beta } = 0$ . The easiest way to establish Eq. (12.40) is therefore to refine the harmonic gauge to the TT gauge of Sec. 11.1.5, in which $h ^ { 0 0 } = O ( \lambda _ { c } ^ { 2 } / R ^ { 2 } )$ , $h ^ { 0 j } = O ( \lambda _ { c } ^ { 2 } / R ^ { 2 } )$ , and $h ^ { j k } = h _ { \mathrm { T T } } ^ { j k } + O ( \lambda _ { c } ^ { 2 } / R ^ { 2 } )$ ; the properties $N _ { j } h _ { \mathrm { T T } } ^ { j k } = 0 = \delta _ { j k } h _ { \mathrm { T T } } ^ { j k }$ simplify the calculations significantly.
...
of the TT field in terms of the transverse basis provided by the unit vectors $\vartheta : =$ [cos $\vartheta$ cos $\varphi$ , cos $\vartheta$ sin $\varphi , - \sin \vartheta ]$ and $\varphi : = [ - \sin \varphi , \cos \varphi , 0 ]$ ; these were introduced in Eqs. (11.37) and (11.38) and they are orthogonal to $N : = [ \sin \vartheta \cos \varphi , \sin \vartheta \sin \varphi , \cos \vartheta ]$ . After inserting this decomposition within Eqs. (12.42) and (12.43) we obtain
...
$$
\begin{array} { c } { { ( - g ) t _ { \mathrm { L L } } ^ { j k } = \displaystyle \frac { c ^ { 3 } } { 1 6 \pi G } \bigg \{ N ^ { j } \left( \partial _ { \tau } h ^ { p q } \right) \left( \ddot { \Omega } _ { p } h ^ { k q } - \frac 1 2 \ddot { \Omega } ^ { k } h _ { p q } \right) + N ^ { k } \left( \partial _ { \tau } h ^ { p q } \right) \left( \ddot { \Omega } _ { p } h ^ { j q } - \frac 1 2 \ddot { \Omega } ^ { j } h _ { p q } \right) } } \\ { { - N ^ { p } \bigg [ \left( \partial _ { \tau } h ^ { j q } \right) \left( \ddot { \Omega } _ { p } h ^ { k q } \right) + \left( \partial _ { \tau } h ^ { k q } \right) \left( \ddot { \Omega } _ { p } h ^ { j q } \right) \bigg ] - \displaystyle \frac { 1 } { R } \left( h ^ { j q } \partial _ { \tau } h ^ { k q } + h ^ { k q } \partial _ { \tau } h ^ { j q } \right) } } \end{array}
$$
...
$$
\Im ^ { j k } = \frac { c ^ { 3 } } { 1 6 \pi G } \int R ^ { 3 } \big ( \partial _ { \tau } h ^ { p q } \big ) \bigg [ N ^ { j } \ddot { \partial } _ { p } h ^ { k q } - N ^ { k } \ddot { \partial } _ { p } h ^ { j q } - \frac { 1 } { 2 } \big ( N ^ { j } \ddot { \partial } ^ { k } - N ^ { k } \ddot { \partial } ^ { j } \big ) h _ { p q } \bigg ] d \Omega .
$$

To bring Eq. (12.54) to the standard form of Eq. (12.47), it is helpful to decompose the differential operator $\overrightarrow { \mathrm { O } } _ { j }$ into longitudinal and transverse pieces according to $\Pr _ { j } =$ $N _ { j } \mathfrak { I } _ { R } + \mathfrak { J } _ { j } ^ { \mathrm { T } }$ , where $\preccurlyeq _ { R } : = N ^ { j } \preccurlyeq _ { j }$ is a partial derivative with respect to $R$ , while $\mathfrak { J } _ { j } ^ { \mathrm { T } } : = P _ { j } ^ { \ k } \mathfrak { J } _ { k }$ is the transverse derivative (related to partial derivatives with respect to $\vartheta$ and $\varphi$ ). Because $N _ { p } h ^ { p q } = 0$ , it is easy to see that $\Im _ { R }$ plays no role in the first two terms of Eq. (12.54). The next step is to integrate these terms by parts. We write

$$
\left( \partial _ { \tau } h ^ { p q } \right) N ^ { j } \ddot { \partial } _ { p } ^ { \top } h ^ { k q } = \ddot { \partial } _ { p } ^ { \top } \left( \partial _ { \tau } h ^ { p q } N ^ { j } h ^ { k q } \right) - \partial _ { \tau } \left( \vec { \partial } _ { p } ^ { \top } h ^ { p q } \right) N ^ { j } h ^ { k q } - \left( \partial _ { \tau } h ^ { p q } \right) \left( \vec { \partial } _ { p } ^ { \top } N ^ { j } \right) h ^ { k q } ,
$$

and observe that in the TT gauge, the harmonic condition $\partial _ { \alpha } h ^ { \alpha q } = 0$ yields

$$
\begin{array} { l } { { \partial _ { p } h ^ { p q } = - N _ { p } \partial _ { \tau } h ^ { p q } + N _ { p } \vec { \partial } _ { R } h ^ { p q } + \vec { \partial } _ { p } ^ { \Gamma } h ^ { p q } } } \\ { { \ \qquad = - N _ { p } R ^ { - 1 } h ^ { p q } + \vec { \partial } _ { p } ^ { \Gamma } h ^ { p q } } } \\ { { \ \qquad = \vec { 0 } _ { p } ^ { \Gamma } h ^ { p q } } } \\ { { \ \qquad = 0 . } } \end{array}
$$

With $\mathfrak { J } _ { p } ^ { \mathrm { T } } N ^ { j } = P _ { p } ^ { j } / R$ we find that

$$
\left( \partial _ { \tau } h ^ { p q } \right) N ^ { j } \ddot { \partial } _ { p } ^ { \mathrm { T } } h ^ { k q } = \ddot { \partial } _ { p } ^ { \mathrm { T } } \big ( \partial _ { \tau } h ^ { p q } N ^ { j } h ^ { k q } \big ) - \frac { 1 } { R } \big ( \partial _ { \tau } h ^ { j p } \big ) h ^ { k p } ,
$$
...
$$
\begin{array} { r } { R \partial _ { k } \vartheta ^ { j } = - N ^ { j } \vartheta _ { k } + \cot \vartheta \varphi ^ { j } \varphi _ { k } , } \\ { R \partial _ { k } \varphi ^ { j } = - N ^ { j } \varphi _ { k } - \cot \vartheta \vartheta ^ { j } \varphi _ { k } , } \end{array}
$$

and $e _ { z }$ appears in Eq. (12.49) by virtue of the identity $\begin{array} { r } { \pmb { e } _ { z } = \cos \vartheta N - \sin \vartheta \vartheta } \end{array}$ .

The manipulations that lead to Eq. (12.50) involve relations $x \partial _ { y } - y \partial _ { x } = \partial _ { \varphi }$ , $y \partial _ { z } - z \partial _ { y }$ $= - \sin \varphi \partial _ { \vartheta } - \cot \vartheta \cos \varphi \partial _ { \varphi }$ , and $z \partial _ { x } - x \partial _ { z } = \cos \varphi \partial _ { \vartheta } - \cot \vartheta \sin \varphi \partial _ { \varphi }$ , which are well known from the theory of orbital angular momentum in quantum mechanics.

# 12.2.5 Isaacson’s effective energy-momentum tensor

In a series of two papers published in 1968, Richard Isaacson formulated and developed the shortwave approximation of Sec. 12.2.2. His formulation was more general and ambitious than what was presented there. Instead of limiting his attention to the far-away wave zone of an asymptotically-flat spacetime, Isaacson considered a high-frequency gravitational wave in an arbitrary vacuum region of spacetime in which the radius of curvature $\mathcal { R }$ is large compared to $\lambda _ { c }$ , and developed a systematic expansion of the field equations in powers of $\lambda _ { c } / \mathcal { R }$ . Writing the exact Einstein tensor as $G ^ { \alpha \beta } = G _ { 0 } ^ { \alpha \beta } + G _ { 1 } ^ { \alpha \beta } + G _ { 2 } ^ { \alpha \beta } + \cdot \cdot \cdot$ , where $G _ { 0 } ^ { \alpha \beta }$ is the Einstein tensor of the background spacetime, $G _ { 1 } ^ { \alpha \beta }$ the correction of order $\lambda _ { c } / \mathcal { R }$ , and $G _ { 2 } ^ { \alpha \beta }$ the correction of order $( { \lambda _ { c } } / { \mathcal { R } } ) ^ { 2 }$ , he showed that $G _ { 1 } ^ { \alpha \beta } = 0$ produces a linearized propagation equation for the gravitational waves, while
...
$$
{ ( \mathrm { { r T } ) } ^ { j k } } _ { p q } : = P _ { p } ^ { j } P _ { q } ^ { k } - \frac { 1 } { 2 } { P ^ { j k } } P _ { p q } , \qquad P _ { k } ^ { j } : = \delta _ { k } ^ { j } - N ^ { j } N _ { k } .
$$
...
where the equality $\mathrm { s i g n } \mathop { = } ^ { \mathrm { \scriptsize ~ \mathrm { \scriptsize ~ { ~ \mathrm { ~ \scriptsize ~ { ~ \alpha ~ } ~ } } } } }$ , first introduced near the end of Sec. 11.3.1, indicates that terms proportional to $\delta ^ { j k } , N ^ { j }$ , or $N ^ { k }$ have been discarded; these will not survive the TT projection of Eq. (12.64). Note that the gravitational-wave field involves only the STF pieces of the radiative multipole moments.
...
We shall sketch the steps leading to these results in a moment, but we first observe that our result for $\mathcal { P }$ is the famous quadrupole formula which relates the energy radiated in gravitational waves to the quadrupole-moment tensor of the matter distribution. (Refer to $\mathrm { B o x } 1 1 . 2$ for a discussion of the historical controversy that has surrounded this formula.) This scales as $G / c ^ { 5 }$ , and additional contributions from higher-order multipole moments would appear at order $G / c ^ { 7 }$ and higher. Our result for $\mathcal { F } ^ { j }$ involves a coupling between the mass quadrupole and octupole moments, as well as one between the mass and current quadrupole moments; we see that $\mathcal { F } ^ { j }$ scales as $G / c ^ { 7 }$ instead of $G / c ^ { 5 }$ . Our result for the angular-momentum flux can also be expressed in terms of the torque vector $\begin{array} { r } { \mathcal { T } ^ { j } : = \frac { 1 } { 2 } \epsilon _ { \ p q } ^ { j } \mathcal { T } ^ { p q } } \end{array}$ ,
...
$$
\mathcal { P } = \frac { G } { 2 c ^ { 5 } } \Big ( \delta _ { p r } \delta _ { q s } - 2 \delta _ { p r } \big \langle \big \langle N _ { q } N _ { s } \big \rangle \big \rangle + \frac { 1 } { 2 } \big \langle \big \langle N _ { p } N _ { q } N _ { r } N _ { s } \big \rangle \big \rangle \Big ) f ^ { p q } f ^ { r s } ,
$$
...
$$
\begin{array} { c } { { \displaystyle  N _ { j } N _ { k }   = \displaystyle \frac { 1 } { 3 } \delta _ { j k } , } } \\ { { \displaystyle   N _ { j } N _ { k } N _ { p } N _ { q }   = \displaystyle \frac { 1 } { 1 5 } \big ( \delta _ { j k } \delta _ { p q } + \delta _ { j p } \delta _ { k q } + \delta _ { j q } \delta _ { k p } \big ) , } } \\ { { \displaystyle   N _ { j } N _ { k } N _ { p } N _ { q } N _ { r } N _ { s }   = \displaystyle \frac { 1 } { 1 0 5 } \big ( \delta _ { j k } \delta _ { p q } \delta _ { r s } + \mathrm { d i s t i n c t ~ p e r m u t a t i o n s } \big ) , } } \end{array}
$$
...
$$
{ \mathcal { F } } ^ { j } = { \frac { G } { 3 c ^ { 7 } } } \Big ( \delta _ { p r } \delta _ { q s } \big \{ \big ( N ^ { j } N _ { n } \big ) \big \} - 2 \delta _ { p r } \big \langle \big ( N ^ { j } N _ { q } N _ { s } N _ { n } \big ) \big \rangle + { \frac { 1 } { 2 } } \big \langle \big \langle N ^ { j } N _ { p } N _ { q } N _ { r } N _ { s } N _ { n } \big \rangle \big \rangle \Big ) f ^ { p q } f ^ { r s n } ,
$$

with $f ^ { j k } : = \overleftrightarrow { I } ^ { \langle j k \rangle }$ and $f ^ { j k n } : = \ddot { I } ^ { \cdots ( j k n ) } + 2 \epsilon ^ { m n j } \ddot { J } ^ { \langle m k \rangle } + 2 \epsilon ^ { m n k } \ddot { J } ^ { \langle m j \rangle }$ . After evaluation of the angular integrals and simplification (taking advantage of the STF properties of $f ^ { j k }$ and $f ^ { j k n } )$ , we obtain
...
$$
\begin{array} { r l } & { \tilde { f } ^ { \lambda , k } = 2 \eta m \left( \nu ^ { j } \nu ^ { \lambda } + \frac { G m } { r ^ { 3 } } r ^ { j } \frac { \varepsilon ^ { \lambda } } { r ^ { 5 } } \right) } \\ & { \quad = 2 \eta m \left[ ( r ^ { 2 } - G m / r ) n ^ { j } n ^ { \lambda } + r ( r \dot { \sigma } ) ( n ^ { j } \lambda ^ { k } + \lambda ^ { j } n ^ { k } ) + ( r \dot { \sigma } ) ^ { 2 } \lambda ^ { j } \lambda ^ { k } \right] } \\ & { \quad \quad = 2 \eta m \frac { G m } { p } \left[ - ( 1 + e \cos \psi - e ^ { 2 } \sin ^ { 2 } \psi ) n ^ { j } n ^ { k } \right. } \\ & { \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad + e \sin \phi ( 1 + e \cos \psi ) \left( n ^ { j } \lambda ^ { k } + \lambda ^ { j } n ^ { k } \right) + ( 1 + e \cos \psi ) ^ { 2 } \lambda ^ { k } k ^ { k } \right] , \quad \ ( 1 ) } \\ & { \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad } \\ & { \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad } \\ & { \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad } \\ & { \quad \quad = - 2 \eta m \frac { G m } { r ^ { 3 } } \left[ \frac { 1 } { r ^ { 3 } } ( r ^ { j } x ^ { k } + 3 r ^ { j } \dot { r } ^ { k } ) - \frac { 3 r ^ { j } } { r ^ { 2 } } r ^ { k } \right] } \\ & { \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad } \\ &  \quad \quad \quad \quad = - 2 \eta m \frac { G ( m ) ^ { 3 / 2 } } { p ^ { 5 / 2 } } ( 1 + e \cos \psi ) \left( \varepsilon \sin \psi n ^ { j } n ^ { k } + 3 l \end{array}
$$

$$
\begin{array} { r l } & { = \Delta \eta \eta \frac { G m } { r ^ { 2 } } \Bigg [ \frac { 2 0 } { r } \Bigg ( \nu ^ { 2 } \dot { \nu } ^ { 5 } r ^ { 6 } \nu ^ { 4 } \nu ^ { 6 } + \nu ^ { 7 } \dot { r } ^ { 8 } \nu ^ { 6 } + r ^ { 7 } \nu ^ { 6 } \nu ^ { 8 } \nu ^ { 6 } \bigg ) } \\ & { \qquad - \frac { 3 0 5 } { r ^ { 2 } } \Big ( \nu ^ { 2 } \dot { r } ^ { 6 } \nu ^ { 5 } r ^ { 6 } + \rho ^ { - 1 } \nu ^ { 4 } \rho ^ { - 1 } \dot { r } ^ { 8 } \nu ^ { 6 } + r ^ { 7 } \rho ^ { - 1 } \nu ^ { 6 } \nu ^ { 7 } \Big ) - \frac { 3 } { r ^ { 3 } } \big ( 3 \nu ^ { 2 } - 1 5 ^ { 2 } + 4 \mathcal { G } \mathcal { G } \mathcal { m } / r \big ) \nu ^ { 2 } \dot { r } ^ { 5 } r ^ { 6 } \Bigg ] } \\ & { = \Delta \eta \rho \frac { G m } { r ^ { 2 } } \Bigg \{ 3 \Big [ 2 \nu ^ { 2 } - 3 ( r \dot { \nu } ) ^ { 2 } - 4 \mathcal { G } \mathcal { G } \Big ( \eta \dot { r } ^ { 2 } \nu \Big ) L ^ { 2 } \nu ^ { 6 } \nu ^ { 6 } } \\ & { \qquad + 1 0 \mathcal { G } ( r \dot { \nu } ) \big ( \mu ^ { 3 } r ^ { 5 } \nu ^ { 6 } + n ^ { 7 } \xi \nu ^ { 8 } + \lambda ^ { 2 } \mu ^ { - 1 } \nu ^ { 6 } n ^ { 5 } \big ) } \\ & { \qquad + 2 0 ( r \dot { \nu } ) ^ { 2 } \big ( \nu ^ { 2 } \dot { \nu } ^ { 5 } \nu ^ { 6 } + \lambda ^ { 2 } \mu ^ { - 1 } \nu ^ { 6 } + \lambda ^ { 2 } \lambda ^ { 6 } n ^ { 7 } \big ) \Big ] } \\ & { = \Delta \eta \rho \frac { G ( m ) ^ { 2 } } { r ^ { 3 } } \big ( 1 + e \cos \phi \big ) ^ { 2 } \bigg [ - 3 ( 7 + 1 0 e \cos \phi + 5 \sigma ^ { 2 } \cos ^ { 2 } \phi - 2 \nu ^ { 2 } ) \eta ^ { 2 } n ^ { 5 } n ^ { 7 } } \\ &  \qquad + 1 0 e \sin \phi \big ( 1 + e \cos \phi \big ) \big ( \nu ^ { 2 } \dot { \nu } ^ { 5 } + n ^ { 2 } \lambda ^ { 6 } n ^  \end{array}
$$
...
![](images/f32d4f1151ccdacb46c5836c49092d9a8d3f415d53414955f424c29da1c00eb0.jpg)

# Fig. 12.2

The momentum fluxes $\mathcal { F } ^ { \boldsymbol { x } }$ and ${ \mathcal { F } } ^ { y }$ , in units of $- \Delta \eta ^ { 2 } ( c ^ { 4 } / G ) ( G m / c ^ { 2 } p ) ^ { 1 1 / 2 } ,$ as functions of retarded time $\tau$ , in units of the orbital period P. A minus sign is inserted in the unit of momentum flux to reflect the fact that $\Delta < 0$ when $m _ { 1 } < m _ { 2 }$ . As in Fig. 12.1 the orbital eccentricity is set equal to $e = 0 . 7 .$ We see that most of the momentum is emitted when the orbit is near pericenter. The emission in the $\pmb { \chi }$ -direction cancels out over a complete orbital cycle, but the emission in the y-direction builds up coherently.

<table><tr><td>Box 12.2</td><td>Momentum flux and gravitational-wave beaming</td></tr><tr><td></td><td>Our friend Alan Wiseman has proposed a useful electromagnetic analogy to understand why the momentum flux is always in the direction of motion of the lighter body. Because the lighter body has a higher velocity, the energy flux associated with its motion is beamed in the forward direction,just as the electromagnetic radiation from a rapidly moving body is beamed by relativistic effects.Since energy caries momentum,there is a preferential emission of momentum in the direction of motion of the lighter body. This is admittedly an imperfectanalogy,because the energy flux is predominantly quadrupolar, not dipolar,andthus itcannot have a unique direction associated with it. On the other hand, the momentum flux arises from an interference be- tween the quadrupole moment Ijk and the O.5PN corrections in Eq. (12.67), which depend on the octupole and current quadrupole moments; although the latter are not vectors, they have an odd parity,and they can therefore single out a preferred direction.Scaling the energy flux by mc²,the momentum flux by mc,and taking the ratio gives (Fj /mc)/(P/mc²)~ v/c, precisely the kind of v/c effect that gives rise to relativistic beaming.</td></tr></table>
...
semi-major axis along the line of sight. The parameter ˙ω is defined by Eq. (10.17). The parameter γ  is the coefficient [(M1 + 2M2)/m](a3 / Gm) 1/2 (GM2/c 2 a)e that appears in front of sin u in Eq. (10.135). The   
period derivative P˙ is given by Eq. (12.85). The parameter r is the coefficient GM /c2 that appears in front of the logarithmic term in Eq. (10.139), and s = sin ι appears within the logarithm. The numbers in parentheses indicate the error in the last digit.

<table><tr><td>Parameter</td><td>PSR 1913+16</td><td>J0737-3039A</td></tr><tr><td>Keplerian parameters</td><td></td><td></td></tr><tr><td>a sint/c (s)</td><td>2.341782(3)</td><td>1.415032(1)</td></tr><tr><td>Eccentricity, e</td><td>0.6171334(5)</td><td>0.0877775(9)</td></tr><tr><td>Orbital period, P (day)</td><td>0.322997448911(4)</td><td>0.10225156248(5)</td></tr><tr><td> Relativistic parameters</td><td></td><td></td></tr><tr><td>Pericenter advance, ω(° yr-1)</td><td>4.226598(5)</td><td>16.8995(7)</td></tr><tr><td>Redshift/time dilation, y&#x27; (ms)</td><td>4.2992(8)</td><td>0.386(3)</td></tr><tr><td>Period derivative, P (10-12)</td><td>-2.423(1)</td><td>-1.25(2)</td></tr><tr><td>Shapiro delay, r (μs)</td><td></td><td>6.2(3)</td></tr><tr><td> Shapiro delay, s</td><td></td><td>0.9997(4)</td></tr></table>
...
as a meaningful post-Newtonian expansion parameter, defined directly in terms of the total mass $m$ and orbital frequency $\Omega$ . The orbital velocity is then given by $\upsilon / c = \beta [ 1 -$ $\begin{array} { r } { \frac { 1 } { 3 } ( 3 - \eta ) \beta ^ { 2 } + O ( \beta ^ { 4 } ) ] } \end{array}$ , and the orbital radius is determined from $G m / ( c ^ { 2 } r ) = \beta ^ { 2 } [ 1 + { \textstyle { \frac { 1 } { 3 } } } ( 3 -$ $\eta ) \beta ^ { 2 } + O ( \beta ^ { 4 } ) ]$ . The gravitational-wave polarizations are expressed as
...
$$
\begin{array} { r l } { \kappa _ { 1 , 2 } ^ { \mathrm { R } } } & { = \frac { 4 \mathcal { E } } { \epsilon ^ { 2 } } [ \int _ { c _ { 1 } } c _ { 1 } ^ { \mathrm { S } } c _ { 2 } ^ { \mathrm { R } } d z ^ { \mathrm { R } }   \frac { 1 } { 2 } \frac { \partial } { \partial z ^ { \mathrm { R } } } \int _ { c _ { 2 } } c _ { 1 } ^ { \mathrm { S } } d z ^ { \mathrm { R } } - \kappa _ { 1 } ^ { \mathrm { R } } \frac { \partial } { \partial z ^ { \mathrm { R } } } | \partial z ^ { \mathrm { R } } d z ^ { \mathrm { R } } } \\ & { \quad - \frac { 1 } { 8 } \frac { \partial } { \partial z ^ { \mathrm { R } } } \frac { \partial } { \partial z ^ { \mathrm { R } } } \frac { \partial } { \partial z ^ { \mathrm { R } } } \frac { \partial } { \partial z ^ { \mathrm { R } } } d z ^ { \mathrm { R } } ] } \\ & { \quad - \frac { 1 } { 1 2 8 \mathrm { R } ^ { 2 } \mathrm { R } ^ { 2 } \mathrm { R } ^ { 2 } } \Big [ \mathrm { a } \delta _ { 2 } ^ { \mathrm { R } } d z ^ { \mathrm { R } } \Big ] \frac { 2 } { \partial z ^ { \mathrm { R } } } \mathrm { O } \mathrm { O } \mathrm { O } \mathrm { O } \mathrm { O } \mathrm { R } + \kappa _ { 2 } ^ { \mathrm { R } } \mathrm { R } [ \mathrm \mathrm { O R } ^ { 2 } d \mathrm { R } ^ { 2 }  \bigg ] } \\ & { \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad } \\ { \delta _ { 1 , 2 } ^ { \mathrm { R } } } &  = \frac { 4 \mathcal { E } } { \epsilon ^ { 2 } } [ \int _ { c _ { 1 } } c _ { 2 } ^ { \mathrm { R } } d z ^ { \mathrm { R } } ] \frac { 1 } { \hbar \mathrm { R } ^ { 2 } \mathrm { R } ^ { 2 } \mathrm { R } ^ { 2 } } \Bigg [ \mathrm { O R } \bigg ( \kappa _ { 1 } ^ { \mathrm { R } } \bigg . \frac { \mathrm { R } ^ { 2 } } { \mathrm { R } ^ { 2 } } \bigg . \Bigg ] \frac { \partial } { \partial z ^ { \mathrm { R } } } \int _ { c _ { 2 } } c _ { 1 } ^ { \mathrm { R } } d z ^ { \mathrm { R } } + \mathrm \end{array}
$$
...
$$
\begin{array} { r l } & { T ^ { M } ( t ) : = \displaystyle \int _ { \mathcal { A } } c ^ { - 2 } \tau ^ { \mathrm { a m } } ( t , x ) x ^ { k } x ^ { l } d ^ { 3 } x , } \\ & { T ^ { j k m } ( t ) : = \displaystyle \int _ { \mathcal { A } } c ^ { - 2 } \tau ^ { \mathrm { a m } } ( t , x ) x ^ { j } x ^ { k } x ^ { l } d ^ { 3 } x , } \\ & { T ^ { k l } ( t ) : = \displaystyle \int _ { \mathcal { A } } c ^ { - 2 } \tau ^ { \mathrm { a m } } ( t , x ) x ^ { k } { r } ^ { 2 } d ^ { 3 } x , } \\ & { T ^ { k l } ( t ) : = \displaystyle \int _ { \mathcal { A } } c ^ { - 2 } \tau ^ { \mathrm { a m } } ( t , x ) ^ { j } \mathscr { A } ^ { 3 } x , } \\ & { \mathcal { I } ^ { m k } ( t ) : = \displaystyle \epsilon ^ { \mathrm { a m b } } \int _ { \mathcal { A } } c ^ { - 1 } \tau ^ { \mathrm { a b } } ( t , x ) { r } ^ { a } x ^ { k } d ^ { 3 } x , } \\ & { \mathcal { M } ^ { j k m } : = \displaystyle \int _ { \mathcal { A } } c ^ { j k } ( t , x ) { r } ^ { 2 } d ^ { 3 } x . } \end{array}
$$

We explained in $\mathrm { B o x } \ 7 . 1$ how the multipole moments end up appearing in the expansions of $h _ { \mathcal { N } } ^ { \alpha \beta }$ in powers of $c ^ { - 1 }$ , and how these manipulations give rise to additional boundary terms $h ^ { \alpha \beta } [ \partial \mathcal { M } ]$ . All such boundary terms must be carefully examined, and we assert that at all post-Newtonian orders to be considered in this section, the boundary terms contain no $\mathcal { R }$ -independent pieces, and they can therefore be safely discarded. We shall not prove this assertion here, but you may be comforted with the assurance that each boundary term in Eq. (12.101) was examined by us and shown to make no $\mathcal { R }$ -independent contribution to the potentials.
...
$$
\begin{array} { r l } & { V [ 3 ] = - \frac { 1 } { 6 } \frac { \sigma _ { G } ^ { 2 } \kappa _ { \mathrm { R } } } { ( 2 \sqrt { 2 } } [ 6 ] ) , } \\ & { V [ 5 ] = - \mathcal { C } \int _ { \mathcal { H } } \frac { e ^ { \mathrm { i } 5 } } { | x - x | ^ { 2 } } d ^ { 3 } x ^ { \prime } - \frac { 1 } { 6 } \mathrm { e } ^ { \mathrm { i } \sqrt { 2 } k _ { \mathrm { L } } ^ { \mathrm { i } } [ 2 ] } } \\ & { - \frac { 1 } { 1 2 0 } G \left\{ 4 ( \lambda ^ { \mathrm { i } } x ^ { \mathrm { i } } + 2 r ^ { 2 } \sigma ^ { 2 } k _ { \mathrm { i } } ^ { \mathrm { i } } ) \frac { \sigma _ { G } ^ { 3 } } { 2 } [ [ 0 ] ] - 4 r ^ { 5 } \frac { \sigma _ { G } ^ { 5 } \kappa _ { \mathrm { R } } } { 2 } [ [ 0 ] ] + \frac { \sigma _ { G } ^ { 5 } \kappa _ { \mathrm { R } } } { 2 } [ [ 0 ] ] \right\} , } \\ & { V [ 3 ] = \frac { 1 } { 1 8 } G \left\{ 3 \lambda ^ { \mathrm { i } } \frac { \sigma _ { G } ^ { 5 } } { \lambda ^ { \mathrm { i } } } t [ 0 ] - \frac { \sigma _ { G } ^ { 7 } \delta ^ { 6 } } { 7 } [ 0 ] + 2 e ^ { \mathrm { i } \sqrt { 2 } } \frac { \sigma _ { G } ^ { 5 } } { \sqrt { \pi } } [ 0 ] \right\} , } \\ & { W ^ { k } [ 3 ] = - \frac { 1 } { 2 } \frac { \sigma _ { G } ^ { 7 } \kappa _ { \mathrm { R } } } { G ^ { 2 } \lambda ^ { \mathrm { i } } [ 0 ] } , } \\ & { W ^ { k } [ 5 ] = - \frac { 1 } { 2 } \frac { \sigma _ { G } ^ { 7 } \kappa _ { \mathrm { R } } } { ( 2 \sqrt { 2 } ) ^ { k } ( 2 ) } - \frac { 1 } { 3 6 } G \left\{ 3 r ^ { 2 } \frac { \sigma _ { G } ^ { 9 } } { 2 } [ 0 ] - 2 r ^ { 8 } \frac { \sigma _ { G } ^ { 5 } \kappa _ { \mathrm { R } } } { [ 0 ] } [ 0 ] \right. } \end{array}
$$
...
$$
\begin{array} { r l } & { \partial _ { n } { h ^ { 0 0 } } = \frac { 4 } { c ^ { 2 } } \{ \mathfrak { d } _ { n } V [ 0 ] + O ( c ^ { - 2 } ) + c ^ { - \delta } \partial _ { n } V [ 5 ] + O ( c ^ { - \gamma } ) \} , } \\ & { \partial _ { n } { h ^ { 0 j } } = \frac { 4 } { c ^ { 3 } } \{ \mathfrak { d } _ { n } V ^ { j } [ 0 ] + O ( c ^ { - 2 } ) + c ^ { - 3 } \partial _ { n } V ^ { j } [ 3 ] + O ( c ^ { - 5 } ) \} , } \\ & { \partial _ { n } { h ^ { j k } } = \frac { 4 } { c ^ { 4 } } \{ \mathfrak { d } _ { n } W ^ { j k } [ 0 ] + O ( c ^ { - 2 } ) + c ^ { - 3 } \partial _ { n } W ^ { j k } [ 3 ] + O ( c ^ { - 5 } ) \} , } \\ & { \partial _ { 0 } { h ^ { 0 0 } } = \frac { 4 } { c ^ { 3 } } \{ \mathfrak { d } _ { i } V [ 0 ] + O ( c ^ { - 2 } ) + c ^ { - 3 } \partial _ { t } V [ 3 ] + O ( c ^ { - 5 } ) \} , } \\ & { \partial _ { 0 } { h ^ { 0 j } } = \frac { 4 } { c ^ { 4 } } \{ \mathfrak { d } _ { i } V ^ { j } [ 0 ] + O ( c ^ { - 2 } ) + c ^ { - 3 } \partial _ { t } V ^ { j } [ 3 ] + O ( c ^ { - 5 } ) \} , } \\ & { \partial _ { 0 } { h ^ { j k } } = \frac { 4 } { c ^ { 5 } } \{ \mathfrak { d } _ { i } W ^ { j k } [ 0 ] + O ( c ^ { - 2 } ) + c ^ { - 3 } \partial _ { t } W ^ { j k } [ 1 ] + O ( c ^ { - 3 } ) \} , } \end{array}
$$
...
in which $\varrho [ 5 ]$ is expressed as a function of $t$ and $\mathbf { \Delta } _ { \mathbf { { x } ^ { \prime } } }$ . Making the substitution from Eq. (12.120) gives
...
<table><tr><td>Box 12.3</td><td colspan="2">Radiation-reaction potentials</td></tr><tr><td rowspan="3"></td><td>The gravitational potentials in the near zone can be expanded as</td><td></td></tr><tr><td>h00 =</td><td></td></tr><tr><td>hi = hik</td><td>{+( )</td></tr></table>

in which $U : = V [ 0 ] , U ^ { j } : = V ^ { j } [ 0 ] ,$ , and $P ^ { j k } : = W ^ { j k } [ 0 ]$ are the leading-order, near-zone potentials listed in Box 7.5. The terms that come with an odd power of $c ^ { - 1 }$ are the radiation-reaction potentials, and they are given by

$$
\begin{array} { r l } & { ~ F [ 3 ] = - \frac { 1 } { 6 } G \widetilde { F } \dot { u } , } \\ & { ~ F [ 5 ] = G [ - \overline { { \gamma ^ { 2 } } } \widetilde { u } \delta _ { a , b } X - \frac { 1 } { 6 } \widetilde { x } \delta [ 4 ] { 2 } - \frac { 1 } { 6 0 } ( \dot { r } ^ { \gamma \beta } + 2 x ^ { \gamma } x ^ { \beta } ) ^ { 3 } ] \delta ^ { 3 } } \\ & { ~ 1 \frac { 1 } { 3 0 } \frac { \eta ^ { 3 } \eta ^ { 3 } \eta ^ { 4 } } { \eta ^ { 2 } \eta ^ { 5 } } ~ 1 2 \overline { { \eta ^ { 3 } \eta ^ { 3 } } } ] , } \\ & { ~ F ^ { \gamma [ 5 ] } = G [ \frac { 1 } { 5 } \dot { s } ^ { \frac { 3 } { \gamma \beta } \delta } - \frac { 1 } { 1 8 } \frac { \eta ^ { 3 } \delta } { \gamma ^ { 2 } \eta ^ { 5 } } - \frac { 1 } { 6 } \overline { { \gamma ^ { 2 } } } \eta ^ { 3 } ] , } \\ & { ~ H ^ { [ F ] } [ 1 ] = - \frac { 1 } { 2 } G \widetilde { F } ^ { [ F ] } , } \\ & { ~ G ^ { [ F ] } [ 3 ] - G [ - \frac { 1 } { 2 } \frac { 1 } { 1 2 } \widetilde { r } ^ { \beta } \delta ] - \frac { 1 } { 1 8 } r ^ { \gamma \beta } + \frac { 1 } { 1 8 } s ^ { \gamma \beta } \eta ^ { 5 }  } \\ & { ~  ~ + \frac { 1 } { 9 } x ^ { \gamma } ( \widetilde { f } ^ { [ F ] } + \widetilde { \gamma } ^ { [ F ] } ) - \frac { 1 } { 9 } \widetilde { H } ^ { [ F ] } ] . } \end{array}
$$

They are expressed in terms of the Newtonian multipole moments

$$
\begin{array} { r l } & { { I ^ { j k } } : = \displaystyle { \int \rho ^ { * } x ^ { j } x ^ { k } d ^ { 3 } x } , } \\ & { { I ^ { j k n } } : = \displaystyle { \int \rho ^ { * } x ^ { j } x ^ { k } x ^ { n } d ^ { 3 } x } , } \\ & { { I ^ { j k n } } : = \displaystyle { \int \rho ^ { * } x ^ { j } x ^ { k } x ^ { n } x ^ { n } d ^ { 3 } x } , } \\ & { { I ^ { j k n } } : = \displaystyle { \int \rho ^ { * } x ^ { j } x ^ { k } x ^ { n } x ^ { n } d ^ { 3 } x } , } \\ & { { I ^ { j k n } } : = \displaystyle { \int \rho ^ { * } ( v ^ { j } x ^ { k } - v ^ { k } x ^ { j } ) x ^ { n } d ^ { 3 } x } , } \\ & { { I ^ { j k n p } } : = \displaystyle { \int \left( \rho ^ { * } v ^ { j } v ^ { k } + p \delta ^ { j k } \right) x ^ { n } x ^ { p } d ^ { 3 } } . } \end{array}
$$

These are functions of $t$ , and the number of overdots, or the number within an overlaid bracket, indicates the number of differentiations with respect to $t$ . The potentials also depend on $c ^ { - 2 }  { \mathbb { T } } ^ { j k }$ [2], the post-Newtonian correction to the quadrupole moment; an expression for this is not required in subsequent calculations. In addition, $V [ 5 ]$ depends on the post-Newtonian superpotential $X$ , defined by
...
We first determine the pieces of the metric that are involved in the derivation of the radiationreaction force. These are obtained by inserting the potentials of Box 12.3 within Eqs. (7.20). For our purposes here the non-linear terms are important, and after a straightforward computation we obtain

$$
\begin{array} { r l } & { g _ { 0 0 } = - 1 + 2 c ^ { - 2 } U + O ( c ^ { - 4 } ) + 8 c ^ { - 5 } V [ 3 ] + O ( c ^ { - 6 } ) } \\ & { \qquad + 2 c ^ { - 7 } \big ( V [ 5 ] + W [ 3 ] \big ) - 2 4 c ^ { - 7 } V [ 3 ] U + O ( c ^ { - 8 } ) , } \\ & { g _ { 0 i } = O ( c ^ { - 3 } ) + O ( c ^ { - 5 } ) - 4 c ^ { - 6 } V _ { i } [ 3 ] + O ( c ^ { - 7 } ) , } \\ & { g _ { j k } = \delta _ { j k } + O ( c ^ { - 2 } ) + O ( c ^ { - 4 } ) + 4 c ^ { - 5 } \big ( W _ { j k } [ 1 ] - \delta _ { j k } V [ 3 ] \big ) + O ( c ^ { - 6 } ) , } \\ & { g ^ { 0 i } = - 1 - 2 c ^ { - 2 } U + O ( c ^ { - 4 } ) - 8 c ^ { - 5 } V [ 3 ] + O ( c ^ { - 6 } ) } \\ & { \qquad - 2 c ^ { - 7 } \big ( V [ 5 ] + W [ 3 ] \big ) - 8 c ^ { - 7 } V [ 3 ] U + O ( c ^ { - 8 } ) , } \\ & { g ^ { 0 j } = O ( c ^ { - 3 } ) + O ( c ^ { - 5 } ) - 4 c ^ { - 6 } V ^ { j } [ 3 ] + O ( c ^ { - 7 } ) , } \\ & { g ^ { j k } = \delta ^ { j k } + O ( c ^ { - 2 } ) + O ( c ^ { - 4 } ) - 4 c ^ { - 5 } \big ( W ^ { j k } [ 1 ] - \delta ^ { j k } V [ 3 ] \big ) + O ( c ^ { - 6 } ) , } \\ & { { \sqrt { - g } } = 1 + O ( c ^ { - 2 } ) + O ( c ^ { - 4 } ) - 4 c ^ { - 5 } V [ 3 ] + O ( c ^ { - 6 } ) . } \end{array}
$$
...
$$
\begin{array} { l } { { c ^ { 2 } \Gamma _ { 0 0 } ^ { j } = - \partial _ { j } U + O ( c ^ { - 2 } ) + O ( c ^ { - 4 } ) - c ^ { - 5 } } } \\ { { \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \left( { } { } \partial ^ { j } V [ 5 ] + { \partial } ^ { j } W [ 3 ] + { 4 } { \partial } _ { t } V ^ { j } [ 3 ] - { 4 } W ^ { j k } [ 1 ] { \partial } _ { k } U - 8 V [ 3 ] { \partial } ^ { j } U \right) + O ( c ^ { - 6 } ) , } }  \\ { { \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \left( { } { } 1 2 . 1 2 9 \mathrm { } \mathrm { } \mathrm { } \mathrm { } \right)} \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { }  { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \mathrm { } \end{array}
$$
...
$$
\begin{array} { r } { f ^ { j } [ 5 ] : = \rho ^ { * } \partial _ { j } \big ( V [ 5 ] + W [ 3 ] \big ) + 4 \rho ^ { * } \partial _ { t } V ^ { j } [ 3 ] - 4 \rho ^ { * } v ^ { k } \big ( \partial _ { j } V _ { k } [ 3 ] - \partial _ { k } V _ { j } [ 3 ] + \partial _ { t } W _ { j k } [ 1 ] \big ) } \\ { - 4 \rho ^ { * } W ^ { j k } [ 1 ] \partial _ { k } U - 8 \rho ^ { * } V [ 3 ] \partial _ { j } U + 4 W ^ { j k } [ 1 ] \partial _ { k } p + 4 V [ 3 ] \partial _ { j } p . \qquad ( 1 2 . 1 3 6 \textrm { -- } \ ) } \end{array}
$$
...
$$
\begin{array} { l } { { \displaystyle f ^ { j } [ \mathrm { r r } ] = \frac { G } { c ^ { 5 } } \bigg ( - \rho ^ { * } \ddot { I } ^ { p q } \partial _ { j p q } X + 2 \rho ^ { * } \ddot { I } ^ { j k } \partial _ { k } U + \frac { 4 } { 3 } \rho ^ { * } \ddot { I } _ { p p } \partial _ { j } U - 2 \dddot { I } ^ { j k } \partial _ { k } p - \frac { 2 } { 3 } \dddot { I } ^ { p p } \partial _ { j } p } } \\ { { \displaystyle \qquad + 2 \rho ^ { * } \ddot { I } ^ { j k } v _ { k } + \frac { 3 } { 5 } \rho ^ { * } \ddot { I } ^ { j k } x ^ { k } - \frac { 1 } { 5 } \rho ^ { * } \ddot { I } ^ { p p } x ^ { j } - \frac { 2 } { 1 5 } \rho ^ { * } \overset { ( 5 ) } { I } ^ { j p p } - \frac { 2 } { 3 } \rho ^ { * } \ddot { I } ^ { j p p } \bigg ) } } \\ { { \displaystyle \qquad + O ( c ^ { - 7 } ) . } } \end{array}
$$

The various multipole moments that appear in this expression are defined in $\mathrm { B o x } 1 2 . 3$
...
$$
\begin{array} { l } { { \displaystyle \int \rho ^ { * } v ^ { p } \partial _ { p j k } X d ^ { 3 } x = G \int \rho ^ { * } \rho ^ { * } v ^ { p } \partial _ { p j k } | x - x ^ { \prime } | d ^ { 3 } x ^ { \prime } d ^ { 3 } x } } \\ { { \displaystyle \qquad = G \int \rho ^ { * } \rho ^ { * } v ^ { p } \partial _ { p ^ { \prime } j k } | x ^ { \prime } - x | d ^ { 3 } x d ^ { 3 } x ^ { \prime } } } \\ { { \displaystyle \qquad = - G \int \rho ^ { * } \rho ^ { * } v ^ { p } \partial _ { p j k } | x - x ^ { \prime } | d ^ { 3 } x ^ { \prime } d ^ { 3 } x } } \\ { { \displaystyle \qquad = \frac { 1 } { 2 } G \int \rho ^ { * } \rho ^ { * } ( v - v ^ { \prime } ) ^ { p } \partial _ { p j k } | x - x ^ { \prime } | d ^ { 3 } x ^ { \prime } d ^ { 3 } x } } \\ { { \displaystyle \qquad = \frac { 1 } { 2 } G \frac { d } { d t } \int \rho ^ { * } \rho ^ { * } \partial _ { k } | x - x ^ { \prime } | d ^ { 3 } x ^ { \prime } d ^ { 3 } x } } .  \end{array}
$$

We swap the identities of the integration variables to go from the first to the second line, and to go to the third line we observe that $\partial _ { j ^ { \prime } } | { \pmb x } - { \pmb x } ^ { \prime } | = - \partial _ { j } | { \pmb x } - { \pmb x } ^ { \prime } |$ . We symmetrize the expressions in the fourth line, and in the fifth line we invoke an integral identity derived in Box 8.4. The final result of Eq. (12.144b) follows after evaluating the remaining derivatives.
...
$$
\begin{array} { l } { { { \displaystyle { \frac { c ^ { 5 } } { G } } \int v _ { j } f ^ { j } [ \mathrm { r r } ] d ^ { 3 } x = - \ddot { T } _ { j k } \bigl ( 2 \dot { \mathcal { T } } ^ { j k } + \dot { \Omega } ^ { j k } \bigr ) + { \frac { 1 } { 3 } } \ddot { T } _ { p p } \bigl ( 2 \dot { \mathcal { T } } + \dot { \Omega } \bigr ) } } } \\ { { { } + { \frac { 3 } { 1 0 } } { I } ^ { ( j k } \dot { I } ^ { j k } - { \frac { 1 } { 1 0 } } \overset { ( s ) } { I } ^ { p p } \dot { I } ^ { p p } + 4 { \frac { d } { d t } } \Bigl ( \ddot { I } ^ { \dot { \jmath } k } \mathcal { T } ^ { j k } \Bigr ) } } \end{array}
$$
...
$$
\begin{array} { l } { { { \displaystyle \frac { c ^ { s } } { G } \int v _ { j } f ^ { j } [ \mathrm { r r } ] d ^ { 3 } x = - \frac { 1 } { 5 } \dddot { I } ^ { \prime } \ddot { I } ^ { \prime } \ddot { I } ^ { j k } + \frac { 1 } { 1 5 } \dddot { I } ^ { \prime \prime p p } \ddot { I } ^ { \mathrm { r p p } } + \frac { d } { d t } \biggl ( 4 \dddot { I } ^ { \prime \prime k } \mathcal { T } ^ { j k } + \frac { 3 } { 1 0 } \stackrel { ( i ) } { I ^ { j k } } \dot { I } ^ { j k } - \frac { 1 } { 1 0 } \stackrel { ( i ) } { I ^ { p p } } \dot { I } ^ { p p } + } } } \\ { { { \displaystyle \qquad - \frac { 3 } { 1 0 } \ddot { I } ^ { \prime \prime k } \ddot { I } ^ { j k } + \frac { 1 } { 1 0 } \dddot { I } ^ { \prime p p } \ddot { I } ^ { p p } \biggr ) } } } \end{array}
$$
...
which is our precise statement of momentum balance. We must verify that the integrated radiation-reaction force is equal to a total time derivative, and the computation will reveal the identity of $P ^ { j } [ 5 ]$ .
...
$$
\frac { d J ^ { j k } } { d t } = - \Im ^ { j k } = - \frac { 2 G } { 5 c ^ { 5 } } \Big ( \ddot { I } ^ { j p } \dddot { I } ^ { k p } - \ddot { I } ^ { k p } \dddot { I } ^ { j p } \Big ) + { \cal O } ( c ^ { - 7 } ) ,
$$
...
$$
\int \Bigl ( x ^ { j } f ^ { k } [ \mathrm { r r } ] - x ^ { k } f ^ { j } [ \mathrm { r r } ] \Bigr ) d ^ { 3 } x + \frac { d } { d t } \Bigl ( c ^ { - 5 } J ^ { j k } [ 5 ] \Bigr ) = - \frac { 2 G } { 5 c ^ { 5 } } \Bigl ( \ddot { I } ^ { j p } \ddot { I } ^ { k p } - \ddot { I } ^ { k p } \ddot { I } ^ { \ddot { J } p } \Bigr ) + { \cal O } ( c ^ { - 7 } ) ,
$$
...
$$
\begin{array} { c } { { { \displaystyle \int \rho ^ { * } x ^ { j } \partial _ { k } U d ^ { 3 } x = \Omega _ { j k } , } } } \\ { { { \displaystyle \int \rho ^ { * } x ^ { j } \partial _ { k p q } X d ^ { 3 } x = \Omega _ { j k } \delta _ { p q } + \Omega _ { j p } \delta _ { k q } + \Omega _ { j q } \delta _ { k p } - 3 \Omega _ { j k p q } } , } } \\ { { { \displaystyle \int \rho ^ { * } \partial _ { k } p d ^ { 3 } x = - \delta _ { j k } P } , } } \\ { { { \displaystyle \int \rho ^ { * } x ^ { j } \nu ^ { k } d ^ { 3 } x = \frac { 1 } { 2 } ( { \dot { I } } ^ { j k } + J ^ { j k } ) + { \cal O } ( c ^ { - 2 } ) , } } } \end{array}
$$
...
$$
\begin{array} { l } { { c ^ { - 5 } J ^ { j k } [ 5 ] = - { \displaystyle \frac { G } { 5 c ^ { 5 } } } \Big [ 3 \big ( I ^ { j p } \ddot { I } ^ { 4 p } - I ^ { k p } \ddot { I } ^ { j p } \big ) + 2 \big ( \dot { I } ^ { j p } \ddot { I } ^ { k p } - \dot { I } ^ { k p } \ddot { I } ^ { \cdots { p } p } \big ) } } \\ { { \qquad + 5 \big ( J ^ { j p } \ddot { I } ^ { \ast p } - J ^ { k p } \ddot { I } ^ { \ast p } \big ) \Big ] . } } \end{array}
$$
...
$$
\begin{array} { l } { { a _ { \cal A } ^ { j } [ \mathrm { r r } ] = { \displaystyle \frac { G } { c ^ { 5 } } } \biggl ( - \ddot { \cal I } ^ { \dot { p } q } \partial _ { j p q } X _ { \neg { \cal A } } + 2 \ddot { \cal I } ^ { \dot { p } k } \partial _ { k } U _ { \neg { \cal A } } + { \displaystyle \frac { 4 } { 3 } } \ddot { \cal I } ^ { p p } \partial _ { j } U _ { \neg { \cal A } } + 2 \stackrel { ( \mathrm { 4 } ) } { \cal I } ^ { \dot { p } k } v _ { A } ^ { k } } } \\ { { \qquad + { \displaystyle \frac { 3 } { 5 } } \stackrel { ( \mathrm { 5 } ) } { \cal I } ^ { \dot { p } k } r _ { A } ^ { k } - { \displaystyle \frac { 1 } { 5 } } \stackrel { ( \mathrm { 5 } ) } { \cal I } ^ { p p } r _ { A } ^ { j } - { \displaystyle \frac { 2 } { 1 5 } } \stackrel { ( \mathrm { 5 } ) } { \cal I } ^ { j p p } - { \displaystyle \frac { 2 } { 3 } } \stackrel { ( \mathrm { 4 } ) } { \cal J } ^ { j p p } \biggr ) + { \cal O } ( c ^ { - 7 } ) , } } \end{array}
$$
...
$$
\begin{array} { r } { a _ { A } ^ { j } [ \mathrm { r r } ] = \displaystyle \frac { G } { c ^ { 5 } } \bigg ( - 3 \dddot { I } ^ { p q } \sum _ { B \ne A } \frac { G M _ { B } } { r _ { A B } ^ { 2 } } n _ { A B } ^ { j } n _ { A B } ^ { p } n _ { A B } ^ { q } - \frac { 1 } { 3 } \dddot { I } ^ { p p } \sum _ { B \ne A } \frac { G M _ { B } } { r _ { A B } ^ { 2 } } n _ { A B } ^ { j } } \\ { + 2 _ { } ^ { ( 4 ) j k } v _ { A } ^ { k } + \frac { 3 } { 5 } \hat { I } ^ { \langle j k \rangle } r _ { A } ^ { k } - \frac { 2 } { 1 5 } \hat { I } ^ { \langle j p p } - \frac { 2 } { 3 } \dddot { J } ^ { \langle j p p } \bigg ) + O ( c ^ { - 7 } ) . } \end{array}
$$
...
The mass quadrupole moment is $I ^ { j k } = \eta m r ^ { j } r ^ { k }$ , where $\eta : = { \cal M } _ { 1 } { \cal M } _ { 2 } / m ^ { 2 }$ is the symmetric mass ratio, and to evaluate its derivatives we rely on the Keplerian dynamics reviewed in Sec. 3.2. From the Newtonian equation of motion $\pmb { a } = - G m \pmb { n } / r ^ { 2 }$ we deduce that $v \dot { v } =$ $- G m { \dot { r } } / r ^ { 2 }$ and $r \ddot { r } = v ^ { 2 } - { \dot { r } } ^ { 2 } - G m / r$ , up to corrections of order $c ^ { - 2 }$ , and a straightforward computation reveals that

$$
\begin{array} { c } { { \widetilde { I } ^ { j k } = - 2 \eta m \displaystyle \frac { G m } { r ^ { 2 } } \Big [ 2 \Big ( n ^ { j } v ^ { k } + v ^ { j } n ^ { k } \Big ) - 3 \dot { r } n ^ { j } n ^ { k } \Big ] , } } \\ { { \overset { ( 4 ) } { \underset { J ^ { j k } } { = } } = - 2 \eta m \displaystyle \frac { G m } { r ^ { 3 } } \Big [ - 9 \dot { r } \Big ( n ^ { j } v ^ { k } + v ^ { j } n ^ { k } \Big ) + 4 v ^ { j } v ^ { k } } } \\ { { + \left( 1 5 \dot { r } ^ { 2 } - 3 v ^ { 2 } - G m / r \right) n ^ { j } n ^ { k } \Big ] , } } \\ { { \overset { ( 5 ) } { \underset { J ^ { j k } } { = } } = - 2 \eta m \displaystyle \frac { G m } { r ^ { 4 } } \Big [ 4 \Big ( 1 5 \dot { r } ^ { 2 } - 3 v ^ { 2 } + G m / r \Big ) \Big ( n ^ { j } v ^ { k } + v ^ { j } n ^ { k } \Big ) } } \\ { { - 3 0 \dot { r } v ^ { j } v ^ { k } + 1 5 \dot { r } \Big ( 3 v ^ { 2 } - 7 \dot { r } ^ { 2 } \Big ) n ^ { j } n ^ { k } \Big ] . } } \end{array}
$$
...
$$
\begin{array} { l } { { \bar { g } _ { 0 j } [ 6 ] = 0 , } } \\ { { \bar { g } _ { j k } [ 5 ] = 2 a G \ddot { I } ^ { \ddot { \langle j k \rangle } } + 2 b \delta _ { j k } G \ddot { I } ^ { \ddot { \langle p p } } , } } \end{array}
$$
...
According to Eq. (12.177) and the potentials of Box 12.3, to achieve the required form for $\bar { g } _ { j k }$ [5] we need $h ^ { j }$ to be a solution to

$$
\partial _ { \bar { \jmath } } h _ { k } + \partial _ { \bar { k } } h _ { j } = 2 ( 1 + a ) G \ddot { \bar { I } } ^ { \ddot { \jmath } j k ) } + 2 b \delta _ { j k } G \ddot { \bar { I } } ^ { \ddot { \jmath } p p } .
$$
...
$$
G ^ { - 1 } \partial _ { \bar { \jmath } } \beta = \dot { f } _ { j } + \frac { 2 } { 9 } I ^ { \mathit { 0 } j p p } + \frac { 4 } { 9 } \ddot { J } ^ { \mathit { j } p p } + \biggl [ \biggl ( \frac { 1 } { 3 } + a \biggr ) ^ { \langle \mathit { j } \rangle } + \biggl ( b - \frac { 2 } { 9 } \biggr ) \delta _ { j k } ^ { \ \langle \mathit { j } \rangle } J ^ { \mathit { 1 } \bar { x } } ,
$$

and in this case we find that

$$
\begin{array} { l } { { G ^ { - 1 } \beta = \gamma + \Big ( \dot { f } _ { j } + \frac { 2 } { 9 } I ^ { ( j p p } + \frac { 4 } { 9 } \dddot { J } ^ { j p p } \Big ) \bar { x } ^ { j } } } \\ { { \qquad + \frac { 1 } { 2 } \bigg [ \bigg ( \frac { 1 } { 3 } + a \bigg ) _ { I } ^ { ( 4 ) } { } ^ { } { } ^ { j k } \nonumber + \bigg ( b - \frac { 2 } { 9 } \bigg ) \delta _ { j k } { } ^ { ( 4 ) } { } ^ { p p } \bigg ] \bar { x } ^ { j } \bar { x } ^ { k } , } } \end{array}
$$
...
$$
\Delta U [ 5 ] = - ( 1 + a ) G \overleftrightarrow { I } ^ { \langle j k \rangle } \partial _ { \bar { \jmath } \bar { k } } \bar { X } + b G \overleftrightarrow { I } ^ { \cdot p p } \bar { U }
$$
...
$$
\begin{array} { c } { { \displaystyle \mathcal { U } = - \frac { 2 G } { 5 c ^ { 5 } } \ddot { I } ^ { \langle j k \rangle } \partial _ { j k } X , } } \\ { { \displaystyle \mathcal { V } _ { j k } = - \frac { 2 G } { 5 c ^ { 5 } } \ddot { I } ^ { \langle j k \rangle } ; } } \end{array}
$$
...
$$
\begin{array} { l } { { c ^ { - 2 } \sqrt { - g } T ^ { 0 0 } = \rho ^ { * } + { \cal O } ( c ^ { - 7 } ) , } } \\ { { c ^ { - 1 } \sqrt { - g } T ^ { 0 j } = \rho ^ { * } v ^ { j } + { \cal O } ( c ^ { - 7 } ) , } } \\ { { \sqrt { - g } T ^ { j k } = \rho ^ { * } v ^ { j } v ^ { k } + p \delta ^ { j k } - \bigl ( 2 \mathcal { V } ^ { j k } - \delta ^ { j k } \mathcal { V } \bigr ) p + { \cal O } ( c ^ { - 7 } ) . } } \end{array}
$$
...
$$
\begin{array} { l } { { f ^ { j } [ \mathrm { r r } ] = \displaystyle \frac { G } { c ^ { \dot { s } } } \bigg \{ a \ddot { \cal I } ^ { ( p q ) } \rho ^ { * } \partial _ { j p q } X - 2 a \stackrel { \cdots \ddot { \cal I } ^ { ( j k ) } } { \cal I } ^ { ( p k ) } \big ( \rho ^ { * } \partial _ { k } U - \partial _ { k } p \big ) - b \stackrel { \cdots \ddot { \cal I } ^ { p p } } { \cal I } ^ { ( p p } \big ( 3 \rho ^ { * } \partial _ { j } U + \partial _ { j } p \big ) } } \\ { { \qquad \quad - 2 \rho ^ { * } \bigg ( a \stackrel { ( 4 ) } { \cal I } ^ { ( p k ) } + b \delta _ { j k } \stackrel { ( 4 ) } { \cal I } ^ { p p } \bigg ) v ^ { k } - \rho ^ { * } \bigg [ \bigg ( \frac 2 5 + a \bigg ) \stackrel { ( 5 ) } { \cal I } ^ { ( j k ) } + b \delta _ { j k } \stackrel { ( 5 ) p } { \cal I } ^ { p p } \bigg ] x ^ { k } \bigg \} } } \end{array}
$$
...
$$
\begin{array} { l } { { { a _ { \cal A } ^ { j } [ \mathrm { r r } ] = { \displaystyle \frac { G } { c ^ { 5 } } } \Bigg \{ 3 \sum _ { B \not = A } { \displaystyle \frac { G M _ { B } } { r _ { A B } ^ { 2 } } } \Big ( a \stackrel { \cdots \langle { \it p q } \rangle } { { \cal I } ^ { ( p q ) } } n _ { A B } ^ { p } n _ { A B } ^ { q } + b \stackrel { \cdots \ddot { \it I } ^ { p p } } { { \cal I } ^ { ( p b ) } } \Big ) n _ { A B } ^ { j } } \nonumber } } \\  { { - 2 \Big ( a \stackrel { ( 4 ) } { { \cal I } ^ { ( j k ) } } + b \delta _ { j k } \stackrel { ( 4 ) } { { \cal I } ^ { p p } } \Big ) v _ { A } ^ { k } - \left[ \left( { \displaystyle \frac { 2 } { 5 } } + a \right) \stackrel { ( 5 ) } { { \cal I } ^ { ( j k ) } } + b \delta _ { j k } \stackrel { ( 5 ) } { { \cal I } ^ { p p } } \right] r _ { A } ^ { k } \Bigg \} , } } \end{array}
$$
...
$$
\int v _ { j } f ^ { j } [ \mathrm { r r } ] d ^ { 3 } x + \mathcal { V } \int v ^ { j } \partial _ { j } p d ^ { 3 } x - P ~ \partial _ { t } \mathcal { V } + \frac { d } { d t } \Big ( c ^ { - 5 } E [ 5 ] \Big ) = - \frac { G } { 5 c ^ { 5 } } \dddot { I } ^ { ( j k ) } \ddot { I } ^ { ( j k ) } + O ( c ^ { - 7 } )
$$
...
$$
\begin{array} { c } { { c ^ { - 5 } E [ 5 ] = { \displaystyle \frac { G } { c ^ { 5 } } } \Bigg [ 4 a ~ \overleftrightarrow { I } ^ { \langle j k \rangle } \mathcal { T } ^ { j k } + { \displaystyle \frac { 1 } { 2 } } \Bigg ( \frac { 2 } { 5 } + a \Bigg ) \Big ( \overbrace { I } ^ { ( 4 ) } j k ) ~ \overleftrightarrow { I } ^ { \langle j k \rangle } - \overleftrightarrow { I } ^ { \langle j k \rangle } \overleftrightarrow { I } ^ { \langle j k \rangle } \Bigg ) } } \\ { { + b ~ \overleftrightarrow { I } ^ { \langle p p } \big ( 4 7 + 3 P \big ) + { \displaystyle \frac { 1 } { 2 } } b \left( \overbrace { I } ^ { ( 4 ) } p \dot { I } ^ { q q } - \overleftrightarrow { I } ^ { \langle p p } \ddot { I } ^ { q q } \right) \Bigg ] } } \end{array}
$$
...
We recall that $a$ and $^ b$ are parameters that specify the choice of radiation-reaction gauge. In the Burke–Thorne gauge we set $a = 0$ and $b = 0$ , in the Schafer gauge we set ¨ $a = - 2 / 5$ and $b = 0$ , and the assignments $a = - 2 / 3$ and $b = 4 / 9$ produce the harmonic-gauge expression for the radiation-reaction force.

The effect of a perturbing force on the Keplerian orbital elements $p , e$ , and $\omega$ was described in detail back in Sec. 3.3. In this description, the orbital motion is described at all times by the Keplerian relations of Eq. (12.219), but the orbital elements acquire a time dependence from the perturbing force. This description is exact, and the method of osculating orbits is a powerful starting point for an approximate treatment of the orbital evolution. The relevant equations are displayed in Eqs. (3.69); we have

$$
\begin{array} { l } { \displaystyle \frac { d p } { d f } = \frac { 2 p ^ { 3 } } { G m } \frac { 1 } { ( 1 + e c ) ^ { 3 } } S , } \\ { \displaystyle \frac { d e } { d f } = \frac { p ^ { 2 } } { G m } \bigg [ \frac { s } { ( 1 + e c ) ^ { 2 } } \mathcal { R } + \frac { e + 2 c + e c ^ { 2 } } { ( 1 + e c ) ^ { 3 } } \mathcal { S } \bigg ] , } \\ { \displaystyle e \frac { d \omega } { d f } = \frac { p ^ { 2 } } { G m } \bigg [ - \frac { c } { ( 1 + e c ) ^ { 2 } } \mathcal { R } + \frac { s ( 2 + e c ) } { ( 1 + e c ) ^ { 3 } } \mathcal { S } \bigg ] , } \end{array}
$$

where $c : = \cos f , s : = \sin f , \mathcal { R } : = \pmb { n } \cdot \pmb { a } [ \mathrm { r r } ]$ is the radial component of the perturbing acceleration, and $\begin{array} { r } { S : = \lambda \cdot a [ \mathrm { r r } ] } \end{array}$ is the tangential component. These equations must be supplemented with

$$
\frac { d t } { d f } = \sqrt { \frac { p ^ { 3 } } { G m } } \frac { 1 } { ( 1 + e c ) ^ { 2 } } \Bigg \{ 1 - \frac { 1 } { e } \frac { p ^ { 2 } } { G m } \Bigg [ \frac { c } { ( 1 + e c ) ^ { 2 } } \mathcal { R } - \frac { 2 + e c } { ( 1 + e c ) ^ { 3 } } \sin f \mathcal { S } \Bigg ] \Bigg \} ,
$$

which describes how the orbit evolves in time; this equation can be integrated once the system of Eqs. (12.223) has been solved.

To analyze the equations it is helpful to turn $p$ into a dimensionless variable ${ \mathsf { p } } : = p / p ^ { * }$ by dividing it by a representative scale $p ^ { * }$ ; this may, for example, be chosen as the initial value $p ( f = 0 )$ . It is also helpful to introduce

$$
\epsilon : = \frac { 8 } { 5 } \eta \biggl ( \frac { G m } { c ^ { 2 } p ^ { * } } \biggr ) ^ { 5 / 2 }
$$

as a dimensionless measure of the strength of the radiation-reaction force. Because the orbital velocity $v$ is comparable to $\sqrt { G m / p ^ { * } }$ during the evolution, $\epsilon$ is of the same order of magnitude as $( v / c ) ^ { 5 }$ and therefore quite small. Finally, it is helpful to make time dimensionless by defining $\mathbf { t } : = t / t ^ { * }$ , with $t ^ { * } : = \sqrt { p ^ { * 3 } / ( G m ) }$ .

In terms of these new variables the evolution equations are

$$
\begin{array} { r l } & { \frac { d \mathfrak { p } } { d f } = - 2 \epsilon \mathfrak { p } ^ { - 3 / 2 } ( 1 + e c ) B , } \\ & { \frac { d e } { d f } = \epsilon \mathfrak { p } ^ { - 5 / 2 } ( 1 + e c ) \big [ e s ^ { 2 } A - ( e + 2 c + e c ^ { 2 } ) B \big ] , } \\ & { e \frac { d \omega } { d f } = - \epsilon \mathfrak { p } ^ { - 5 / 2 } s ( 1 + e c ) \big [ e c A + ( 2 + e c ) B \big ] , } \\ & { \frac { d \mathfrak { t } } { d f } = \frac { \mathfrak { p } ^ { 3 / 2 } } { ( 1 + e c ) ^ { 2 } } \bigg \{ 1 - \epsilon \frac { ( 1 + e c ) s } { e \mathfrak { p } ^ { 5 / 2 } } \Big [ e c A + ( 2 + e c ) B \Big ] \bigg \} . } \end{array}
$$

Substitution of $A$ and $B$ returns expressions that are quite large. For our purposes below it is sufficient to present their schematic structure, which is as follows:

$$
\begin{array} { l } { \displaystyle \frac { d \mathsf { p } } { d f } = - \frac { \epsilon } { \mathsf { p } ^ { 3 / 2 } } \bigg [ 8 \bigg ( 1 + \frac { 7 } { 8 } e ^ { 2 } \bigg ) + k _ { 1 } ^ { p } \cos f + k _ { 2 } ^ { p } \cos 2 f + k _ { 3 } ^ { p } \cos 3 f \bigg ] , } \\ { \displaystyle \frac { d e } { d f } = - \frac { \epsilon } { \mathsf { p } ^ { 5 / 2 } } \bigg [ \frac { 3 8 } { 3 } e \bigg ( 1 + \frac { 1 2 1 } { 3 0 4 } e ^ { 2 } \bigg ) + k _ { 1 } ^ { e } \cos f + k _ { 2 } ^ { e } \cos 2 f + k _ { 3 } ^ { e } \cos 3 f } \\ { \displaystyle \qquad + k _ { 4 } ^ { e } \cos 4 f + k _ { 5 } ^ { e } \cos 5 f \bigg ] , } \\ { \displaystyle \frac { d \omega } { d f } = - \frac { \epsilon } { e \mathsf { p } ^ { 5 / 2 } } \bigg [ k _ { 1 } ^ { \omega } \sin f + k _ { 2 } ^ { \omega } \sin 2 f + k _ { 3 } ^ { \omega } \sin 3 f + k _ { 4 } ^ { \omega } \sin 4 f + k _ { 5 } ^ { \omega } \sin 5 f \bigg ] . } \end{array}
$$

The various coefficients $k _ { n }$ that come with the trigonometric functions depend on $e$ as well as the gauge parameters $a$ and $b$ . The schematic forms of Eq. (12.227) reveal that the driving forces for $\mathsf { p }$ and $e$ contain steady pieces that produce secular changes, in addition to oscillatory pieces that average out after each orbital cycle. And as we can see, the oscillatory pieces are gauge-dependent, while the secular pieces are independent of the choice of gauge. The driving force for $\omega$ is entirely oscillatory.

# 12.9.2 Multi-scale analysis of orbital evolution

Apart from the post-Newtonian expansion that gave rise to the radiation-reaction force, no approximations have entered the formulation of the orbital equations as displayed in Eqs. (12.226). We now wish to integrate these equations, and we shall take advantage of the fact that $\epsilon \ll 1$ to find approximate solutions. We must, however, be cognizant of the fact that changes in the orbital elements occur over two very distinct time scales. The first is the orbital time scale $P$ , which is short, and the second is the radiation-reaction time scale $T _ { \mathrm { r r } }$ – refer to Eq. (12.88) – which is longer than $P$ by a factor of order $\epsilon ^ { - 1 } \gg 1$ . In terms of the true anomaly $f$ , we have changes over the short angular scale of $2 \pi$ , and changes over the much longer scale $\epsilon ^ { - 1 }$ . Our perturbative approach must allow us to probe these widely separated scales; the method of choice is a multi-scale analysis, as presented in the excellent book by our friend Carl Bender and his late colleague Steven Orzag (1978), and summarized in Box 12.4.

<table><tr><td>Box 12.4</td><td>Multi-scale analysis</td></tr><tr><td></td><td>To introduce the method we work through a simple example involving a damped harmonic oscilator. We wish to integrate the differential equation x+2∈x+x=0</td></tr><tr><td></td><td>with boundary conditions x(O) = 1,x(O) = O,ina regimein which∈ &lt; 1.Hereall variables are dimen-</td></tr><tr><td></td><td> sionless,andan overdotindicates diferentiation with respecttot.The equation canbe integrated exactly,and</td></tr></table>